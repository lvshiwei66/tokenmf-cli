...
async function resolveParams(
  ...
  const providerMemory = settings.providers[providerName];

  // --- apiKey: --key → settings.json → error ---
  const apiKey = opts.key ?? providerMemory?.apiKey;
  if (!apiKey) {
    throw new TestError(
  ...
  let baseUrl: string | undefined = resolveUrl(providerMemory?.urls);

  if (!baseUrl) {
    const info = await fetchProviderInfo(apiUrl, clientId, providerName);
    if (!("code" in info)) {
  ...
  let model: string | undefined = opts.model ?? providerMemory?.model;
